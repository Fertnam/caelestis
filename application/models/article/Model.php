	...
	class Model {
		...
		public static function getArticlesByPage(int $page, int $count = self::SHOW_BY_DEFAULT) : array {
			try {
				...
				$WritingQuery = $DbConnect->prepare('SELECT * FROM site_article ORDER BY id DESC LIMIT :offset, :count');
		...
		public static function getArticlesCount() : int {
			try {
				...
				$WritingQuery = $DbConnect->query('SELECT COUNT(*) FROM site_article');
